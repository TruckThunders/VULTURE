...
transition_library = np.genfromtxt('atoms.dat',unpack=True,dtype=None)
...
for specie in species:
    ...
    for detection_redshift in line_redshifts[specie]:
        ...
        for i,transition in enumerate(transition_library):
            ...
            for indices in range(index_min,index_max):
                velocity[transition[0]][i] = (LIGHTSPEED * (wav[indices] * wav[indices] - lambda_r * lambda_r)) / (wav[indices] * wav[indices] + lambda_r * lambda_r)
                vel_flux[transition[0]][i] = norm_flux[indices]
                i += 1
...
plt.savefig("{}.eps".format(transition[0]), format='eps')
