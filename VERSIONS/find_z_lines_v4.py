...
try:
    ...
    #ary = np.genfromtxt(filename,dtype=float,delimiter=" ")
    ...
    if ( (MgII_lower > MgII_upper) & (CIV_lower > CIV_upper) ):
        logger.info("========== No wavelength coverage for MgII or CIV, exiting with no detections ==========\n")
        sys.exit("========== No wavelength coverage for MgII or CIV, exiting with no detections ==========\n")

    # If there is no wavelength coverage available, delete the species so we do not loop on this tran
    if ( MgII_lower > MgII_upper ):
        #print " "
        #print "***** No wavelength coverage for MgII - Not searching for MgII *****"
        #print " " 
        logger.warning("***** No wavelength coverage for MgII - Not searching for MgII *****")
        del species['MgII']
        del working['2796']
        del working['2803']
        del noise['2796']
        del noise['2803']
    elif ( (MgII_upper - MgII_lower) < 0.02 ):
        logger.warning("***** Too little wavelength coverage for MgII - Not searching for MgII *****")
        del species['MgII']
        del working['2796']
        del working['2803']
        del noise['2796']
    ...
    if ( CIV_lower > CIV_upper ):
        #print " "
        #print "***** No wavelength coverage for CIV - Not searching for CIV *****"
        #print " " 
        logger.warning("***** No wavelength coverage for CIV - Not searching for CIV *****")
        del species['CIV']    
        del working['1548']
        del working['1551']
        del noise['1548']
        del noise['1551']
    elif ( (CIV_upper - CIV_lower) < 0.02 ):
        logger.warning("***** Too little wavelength coverage for CIV - Not searching for CIV *****")
        del species['CIV']
        del working['1548']
        del working['1551']
        del noise['1548']
    ...
    for specie in species:
        ...
        if ( specie == 'MgII' ):
            ...
            #print "MgII min_z = {} max_z = {}.".format(lower_lim,upper_lim)
            logger.info("MgII blue_limit = {} red_limit = {}".format(blue_limit,red_limit[specie]))
            logger.info("MgII min_z = {} max_z = {}.".format(lower_lim,upper_lim))
        if ( specie == 'CIV' ):
            lower_lim = CIV_lower
            upper_lim = CIV_upper
            #print "CIV blue_limit = {} red_limit = {}".format(blue_limit,red_limit[specie])
            #print "CIV minz_z = {} max_z = {}.".format(lower_lim,upper_lim)
            logger.info("CIV blue_limit = {} red_limit = {}".format(blue_limit,red_limit[specie]))
            logger.info("CIV minz_z = {} max_z = {}.".format(lower_lim,upper_lim))
    ...
    lower_lim = min(MgII_lower,CIV_lower) + 0.01
    upper_lim = max(MgII_upper,CIV_upper) - 0.01

    # Define synthetic line redshift to insert
    fake_redshift = np.arange(lower_lim,upper_lim,((upper_lim-lower_lim)/20.))
    ...
    # Define line center and establish max value
        if ( (shifts > MgII_lower) & (shifts < MgII_upper) ):
            center = np.where(eq_wav == find_nearest(eq_wav,species['MgII']['2796']*(1.+shifts)))
        else:
            center = np.where(eq_wav == find_nearest(eq_wav,species['CIV']['1548']*(1.+shifts)))
        center = int(center[0])
        ...
        if ( (shifts > MgII_lower) & (shifts < MgII_upper) ):
            ...
            height[num_fake_lines],width[num_fake_lines],fake_flux = insert_lines(fake_flux,search_wav,spectral_res,shifts,'CIV',5.*detectionlimit)
        num_fake_lines += 1
    ...
    #print "Min Value Established: {}".format(min_value)
    ...
    #print "Max Value Established: {}".format(max_value)
    ...
    for specie in species:
        ...
        # Line Ratio Check
        if ( specie == 'MgII' ):
            line_redshifts[specie] = line_ratio_check(specie,line_redshifts[specie],scaledpower['2796'],scaledpower['2803'],redshift[specie],0.3)
        if ( specie == 'CIV' ):
            line_redshifts[specie] = line_ratio_check(specie,line_redshifts[specie],scaledpower['1548'],scaledpower['1551'],redshift[specie],0.3)
        ...
        for lines in line_redshifts[specie]:
            ...
            if ( specie == 'MgII' ):
                line_sigma[specie].append(sigma['2796'][fake_centers])
            if ( specie == 'CIV' ):
                line_sigma[specie].append(sigma['1548'][fake_centers])
