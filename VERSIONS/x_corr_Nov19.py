...
def insert_lines(spectrum,wavelengths,resolution,z,tranname,eqwidth):
    ...
    for lambda_r in rest_wavs:
        ...
        b = lambda_r * (1+z)
        c = (b / resolution) * (1. / (2.*sqrt(2.*np.log(2))))
        if (lambda_r == 2796.353786):
            a = eqwidth / (c * sqrt(2. * np.pi))
        if (lambda_r == 2803.530982):
            a = eqwidth / (1.5 * c * sqrt(2. * np.pi))
        npix = (resolution * 4.) / b
        ...
        for i in range(-half_width,half_width):
            spectrum[middle+i] = 1.
            spectrum[middle+i] -= a * exp(-((wavelengths[middle+i] - b)**2.)/(2.*c*c))
...
# Calculate and plot equivalent width detection limit
EWlimit = []
EWlimit = np.array(EWlimit)

newb = species['MgII']['2796'] * (1.+redshift)
EWwidth = (newb / spectral_res) * (1. / (2.*sqrt(2.*np.log(2))))

EWlimit = (np.multiply(5.*noise['2796'],EWwidth) * sqrt(2.*np.pi)) / m
...
for specie in species:
    ...
    for lines in line_redshifts[specie]:
        ...
        line_EWlim[specie].append(EWlimit[fake_centers])
    ...
    #print "--------------------------------"
    #print "Fake Absorption Lines: {}".format(num_fake_lines)
    #print fake_redshift
    #print "--------------------------------"

    #print "--------------------------------"
    #print "Detected {}/{} fake lines.".format(fake_detect,num_fake_lines)
    #print "--------------------------------"
