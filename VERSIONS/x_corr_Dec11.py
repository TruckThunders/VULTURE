...
redshift = np.arange(1.,1.3,delta_z)
...
transition_library = np.genfromtxt('atoms_short.dat',unpack=True,dtype=None)
...
for specie in species:
    ...
    for detection_redshift in line_redshifts[specie]:
        ...
        for transition in transition_library:
            # Don't look at lines in atoms.dat that aren't in the search window
            if (((transition[1]*(1.+detection_redshift)) < blue_limit) | ((transition[1]*(1.+detection_redshift)) > red_limit)):
                continue
            ...
            if ( i % 4 == 0 ):
                if ( i != 0 ):
                    plt.savefig("{}/{}_{}.eps".format(detection_redshift,savedtran,transition[0]), format='eps')
                    plt.close('all')
                savedtran = transition[0]
                plt.figure(figsize=(8.5,11))
                k = 0
            ax[k] = plt.subplot2grid((4,1),(k,0))
            if ( i % 4 == 0 ):
            ...
            plt.plot(velocity[transition[0]],vel_flux[transition[0]],label=transition[0])
            plt.plot(velocity[transition[0]],np.ones(vel_flux[transition[0]].size),'--')
            plt.plot(velocity[transition[0]],np.zeros(vel_flux[transition[0]].size),'--')
            plt.plot([0,0],[0,2],'--')
        ...
        plt.savefig("{}/{}.eps".format(detection_redshift,transition[0]), format='eps')
        ...
        ax1.set_xticklabels(())
        plt.figtext(0.05,0.5,'Relative Flux',ha='center',va='center',size=16,rotation=90)
        plt.plot(velocity['MgII2796'],vel_flux['MgII2796'],label='MgII 2796')
        plt.plot(velocity['MgII2796'],np.ones(vel_flux['MgII2796'].size),'--')
        plt.plot(velocity['MgII2796'],np.zeros(vel_flux['MgII2796'].size),'--')
        plt.legend(frameon=False)
        ...
        plt.plot(velocity['MgII2803'],vel_flux['MgII2803'],label='MgII 2803')
        plt.plot(velocity['MgII2803'],np.ones(vel_flux['MgII2803'].size),'--')
        plt.plot(velocity['MgII2803'],np.zeros(vel_flux['MgII2796'].size),'--')
        plt.legend(frameon=False)
        ...
        plt.plot(velocity['CIV1548'],vel_flux['CIV1548'],label='CIV 1548')
        plt.plot(velocity['CIV1548'],np.ones(vel_flux['CIV1548'].size),'--')
        plt.plot(velocity['CIV1548'],np.zeros(vel_flux['CIV1548'].size),'--')
        plt.legend(frameon=False)
        ...
        plt.xlabel(r'Velocity ($km\,s^{-1}$)',size=16)
        plt.plot(velocity['CIV1551'],vel_flux['CIV1551'],label='CIV1551')
        plt.plot(velocity['CIV1551'],np.ones(vel_flux['CIV1551'].size),'--')
        plt.plot(velocity['CIV1551'],np.zeros(vel_flux['CIV1551'].size),'--')
        plt.legend(frameon=False)
        
        plt.savefig("MgII_CIV.eps", format='eps')
