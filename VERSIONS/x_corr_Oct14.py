...
for transition in noise:
    badvals = np.where((scaledpower[transition] < 1.e-5) & (scaledpower[transition] > -1.e-5))
    noise[transition][badvals[0]] = 0.
...
for specie in species:
    ...
    # Line Ratio Check
    if ( specie == 'MgII' ):
        line_redshifts[specie] = line_ratio_check(line_redshifts[specie],scaledpower['2796'],scaledpower['2803'],redshift,0.5)
    if ( specie == 'CIV' ):
