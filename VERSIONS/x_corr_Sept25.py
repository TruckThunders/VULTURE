...
for spots in gaps_MgII2796:
    # Skip over any chunk less than 5 points in length
    if len(spots) < 5:
        continue
...
for counter,z in enumerate(redshift):
    ...
    for x in range(0,10):
        ...
        threesig = 3. * onesig
        outliers = np.where(chunk2 > threesig)
        for point in outliers[0]:
            chunk2[point] = None
            i = 1
            j = 1
...
for detection in line_redshifts:
    ...
    if ((abs(leftup - rightup) > 3) & (abs(leftdown - rightdown) > 3)):
        line_redshifts = np.delete(line_redshifts,count)
